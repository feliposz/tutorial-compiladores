...
2. Também não vamos discutir tipos definidos pelo usuário. Isto não virá a acontecer até muito mais tarde, pela simples razão de eu não estar convencido de que tipos definidos pelo usuário pertencem a uma linguagem que denominamos KISS. Em capítulos futuros, eu pretendo cobrir pelo menos os conceitos gerais de tipos definidos pelo usuário, registros (*record/struct*), etc., apenas para que a série esteja completa. Mas se eles vão entrar ou não na linguagem KISS ainda é uma questão em aberto.
...
Em todo caso, a ideia nos próximos capítulos da série, é apresentar um conjunto completo de módulos, e compiladores funcionais completos também. O plano, de fato, é ter TRÊS compiladores: Um para a versão de um caractere de TINY (para usar nas experiências), um para TINY completo, e outro para KISS. Neste ponto eu isolei as diferenças entre TINY e KISS, que são as seguintes:

    - TINY terá suporte a apenas dois tipos de dados: o caractere e o inteiro de 16-bits. Eu posso tentar fazer algo com strings também, já que sem elas um compilador não seria de grande utilidade. KISS vai suportar todos os tipos simples usuais, incluindo vetores e até mesmo números de ponto flutuante.
...
Um aviso: os módulos vão continuar sendo escritos para suportar Assembly para processadores 80x86, usando o modelo real de memória e 16-bits. Porém, em todos os programas a geração do código será devidamente encapsulada em uma única unidade, para que qualquer estudante dedicado possa facilmente portar a saída para o processador ou sintaxe que preferir. Esta tarefa é deixada como um "exercício para o estudante".

Mas chega de falação. Vamos começar com o estudo de tipos. Como eu disse anteriormente, vamos tratar disto da mesma forma que no capítulo anterior: fazendo pequenos experimentos usando tokens de um só caractere.
...
A estrutura para a tabela de símbolos para tokens de um só caractere é particularmente simples, e nós já fizemos uso dela diversas vezes antes. Para tratar dela, vamos emprestar algumas rotinas que já usamos antes.
...
Novamente, podemos emprestar a maior parte do código dos programas anteriores. O que segue é uma versão simplificada das versões anteriores destas rotinas. Elas estão muito simplificadas já que eu eliminei coisas como listas de variáveis e inicializadores. Na rotina `AsmAllocVar()`, repare que a chamada a `AddEntry()` também vai cuidar de verificar declarações duplicadas:
...
Enquanto estiver testando, tente declarar duas variáveis com o mesmo nome, e verifique que o analisador avisa a respeito do erro.
...
Agora que podemos declarar variáveis de tipos e tamanhos diferentes, chegamos ao momento em que deveríamos fazer algo de útil com elas. Pra começar, vamos apenas tentar carregá-las no registrador primário (AX no nosso caso). Faz sentido usar a mesma ideia de `AllocVar()`, isto é, fazer uma rotina de carregamento capaz de ler tamanhos diferentes. Também gostaríamos de manter o código dependente de máquina isolado. A rotina de carregamento é algo assim:
...
Seria uma boa ideia testar o programa a esta altura. Como ainda não temos um procedimento para tratar de atribuições ainda, simplesmente adicionamos as linhas:
...
(Vale a pena notar que as novas rotinas que nos permitem manipular tipos são mais simples até do que já fizemos antes. Isto acontece graças à encapsulação da geração de código dos procedimentos.)

Há um pequeno problema. Antes, usávamos o ponto final para sair da rotina `TopDeclarations()`. Agora este é o caractere errado... ele é usado para terminar `Block()`. Em programas anteriores, usamos o símbolo BEGIN (abreviado como "b") para sair. Mas agora ele está sendo usado como um símbolo para tipo.

Há uma solução, que não é das melhores, mas que é simples de fazer. Vamos usar um "B" MAIÚSCULO para representar BEGIN. Então, troque o caractere no laço de `TopDeclarations()`, de "." para "B" e tudo estará bem. Agora, podemos alterar o programa principal para que fique:
...
Só há um pequeno problema: O código gerado está ERRADO!

Observe o código para "a=c" acima. O código é:
...
Para começar, vamos precisar de uma função `GetNum()`. Até agora vimos diversas versões desta versão, algumas retornando apenas um caractere, outras uma string, e outras um valor inteiro. A que vamos usar aqui vai retornar um inteiro longo, para que ela possa tratar qualquer coisa que usarmos. Repare que nenhuma informação de tipo é retornada aqui: `GetNum()` não se importa em como o número será usado:
...
A primeira parte é fácil: podemos **renomear** nossa função existente para `Term()`, como já fizemos tantas vezes antes, e criar uma nova versão para `Expression()`:
...
A ideia geral é que todas as rotinas `AsmPopXXX()` usem esta rotina para obter o primeiro operando. Quando isto é feito, teremos os dois operandos em registradores, para que possamos promover o operando cujo tipo é menor.
...
Segundo, repare que a tabela é simétrica... os dois operandos entram da mesma forma. Finalmente, repare que o produto é SEMPRE um "long", exceto quando ambos operandos são "byte"s. (Vale a pena notar, que muitas expressões vão acabar sendo do tipo "long", queiramos ou não. Talvez nossa ideia de simplesmente promover todos os tipos para "long" não fosse tão ruim assim, no fim das contas!)
...
- Apesar da CPU suportar um dividendo do tipo "long", as instruções providas pelo hardware só são confiáveis para divisão de dividendos tipo "byte" e "word". Para dividendos do tipo "long", precisamos de outra rotina de biblioteca capaz de retornar um resultado "long".
...
Isto foi mudado em FORTRAN IV para suportar aritmética "de tipos misturados". Se uma expressão possuísse algum item real, todos os itens seriam convertidos para reais e a expressão em si seria real. Para completar a figura, funções foram providas para converter explicitamente um tipo em outro, para forçar uma expressão a ter um tipo ou outro.
...
Mesmo assim, Pascal permite algumas conversões implícitas. É possível atribuir um inteiro a um valor real. É possível também misturar os tipos inteiro e real em expressões do tipo Real. Os inteiros seriam automaticamente forçados para o tipo real, da mesma forma que em FORTRAN (e com o mesmo custo em tempo de execução escondido).
...
No espírito de tipagem forte, Pascal não permite que sejam misturadas variáveis do tipo caractere e inteira, sem aplicar as funções explícitas de conversão "Chr" e "Ord".
